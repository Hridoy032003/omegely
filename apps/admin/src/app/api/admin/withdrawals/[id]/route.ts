...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const current = await context.db.from("withdrawal_requests").select("id, user_id, amount_coins, status").eq("id", params.id).single();
  ...
  const updated = await context.db.from("withdrawal_requests").update({ status: nextStatus, admin_note: typeof body.admin_note === "string" ? body.admin_note.slice(0, 240) : null, reviewed_at: new Date().toISOString(), reviewed_by: context.user.id }).eq("id", params.id);

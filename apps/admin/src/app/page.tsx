...
type DashboardData = {
  ...
  withdrawals: Array<{ id: string; user_id: string; amount_coins: number; amount_usd: number; method: string; destination: string; status: string; admin_note: string | null; created_at: string }>;
};
...
function EarningsSection({ data, session }: { data: DashboardData; session: Session }) {
  ...
  return <>
    <section className="kpi-grid earnings-kpis"><KpiCard label="Coins issued" value={data.counts.coinsIssued} hint="$1.00 per 100 coins" icon="$" trend="All reward types" /><KpiCard label="Qualified referrals" value={data.counts.referrals} hint="Successful invites" icon="↗" trend="All time" /></section>
    <AdminCreditForm users={data.users} session={session} />
    <section className="panel table-panel earnings-table"><PanelHeading title="Referral earnings" description="Every qualified referral creates a one-time 100 coin reward ($1.00)." count={`${data.referrals.length} total`} /><div className="table-wrap"><table><thead><tr><th>Referrer</th><th>Referred user</th><th>Reward</th><th>Status</th><th>Created</th></tr></thead><tbody>{data.referrals.map((referral) => <tr key={referral.id}><td><div className="table-user"><span className="table-avatar">{(usersById.get(referral.referrer_id)?.email || "U").slice(0, 1).toUpperCase()}</span><span><strong>{usersById.get(referral.referrer_id)?.email || referral.referrer_id.slice(0, 8)}</strong><small>Code: {referral.referral_code}</small></span></div></td><td>{usersById.get(referral.referred_id)?.email || referral.referred_id.slice(0, 8)}</td><td><strong>{referral.reward_coins} coins · ${(referral.reward_coins / 100).toFixed(2)}</strong></td><td><span className={`badge ${referral.status === "qualified" ? "green" : "muted"}`}>{referral.status}</span></td><td>{formatDate(referral.created_at)}</td></tr>)}{!data.referrals.length && <EmptyTable colSpan={5} text="No referral earnings yet." />}</tbody></table></div></section>
    <section className="panel table-panel earnings-table"><PanelHeading title="Top earners" description="Users with the highest lifetime referral and connection earnings." /><div className="table-wrap"><table><thead><tr><th>User</th><th>Referral code</th><th>Balance</th><th>Lifetime earned</th></tr></thead><tbody>{topEarners.map((user) => <tr key={user.id}><td><strong>{user.email || "Anonymous"}</strong></td><td><span className="mono-label">{user.referral_code || "—"}</span></td><td><strong>{user.coin_balance} coins · ${(user.coin_balance / 100).toFixed(2)}</strong></td><td>{user.total_earned} coins · ${(user.total_earned / 100).toFixed(2)}</td></tr>)}{!topEarners.length && <EmptyTable colSpan={4} text="No earners yet." />}</tbody></table></div></section>
    <section className="panel table-panel earnings-table"><PanelHeading title="Withdrawal queue" description="Review gift-card and cash-pending requests." count={`${data.withdrawals.length} total`} /><div className="table-wrap"><table><thead><tr><th>User</th><th>Amount</th><th>Method</th><th>Destination</th><th>Status</th><th>Action</th></tr></thead><tbody>{data.withdrawals.map((withdrawal) => <tr key={withdrawal.id}><td><span className="mono-label">{usersById.get(withdrawal.user_id)?.email || withdrawal.user_id.slice(0, 8)}</span></td><td><strong>{withdrawal.amount_coins} coins · ${Number(withdrawal.amount_usd).toFixed(2)}</strong></td><td>{withdrawal.method === "gift_card" ? "Omegley gift card" : "Cash pending"}</td><td>{withdrawal.destination}</td><td><span className={`badge ${withdrawal.status === "paid" ? "green" : withdrawal.status === "rejected" ? "red" : "amber"}`}>{withdrawal.status}</span></td><td>{withdrawal.status === "pending" || withdrawal.status === "approved" ? <div className="withdrawal-actions"><button className="tiny" onClick={() => void updateWithdrawal(withdrawal.id, "approved", session.access_token)}>Approve</button><button className="tiny danger" onClick={() => void updateWithdrawal(withdrawal.id, "rejected", session.access_token)}>Reject</button>{withdrawal.status === "approved" && <button className="tiny" onClick={() => void updateWithdrawal(withdrawal.id, "paid", session.access_token)}>Mark paid</button>}</div> : "—"}</td></tr>)}{!data.withdrawals.length && <EmptyTable colSpan={6} text="No withdrawal requests yet." />}</tbody></table></div></section>
  </>;
}

function AdminCreditForm({ users, session }: { users: DashboardData["users"]; session: Session }) {
  const [userId, setUserId] = useState(users[0]?.id || "");
  const [amount, setAmount] = useState("10");
  const [message, setMessage] = useState("Admin bonus");
  const [status, setStatus] = useState("");
  return <section className="panel admin-credit-panel"><PanelHeading title="Issue bonus coins" description="Credit a user with a labeled admin adjustment." /><form onSubmit={(event) => { event.preventDefault(); setStatus("Saving…"); void fetch("/api/admin/earnings/credit", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ user_id: userId, amount_coins: Number(amount), message }) }).then(async (response) => { if (!response.ok) throw new Error((await response.json()).error || "Could not issue coins."); setStatus("Bonus issued. Refreshing…"); window.location.reload(); }).catch((error: Error) => setStatus(error.message)); }} className="admin-credit-form"><select value={userId} onChange={(event) => setUserId(event.target.value)} aria-label="User">{users.map((user) => <option key={user.id} value={user.id}>{user.email || user.id.slice(0, 8)}</option>)}</select><input type="number" min="1" step="1" value={amount} onChange={(event) => setAmount(event.target.value)} aria-label="Coins" /><input value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Reason" aria-label="Bonus message" /><button className="primary-button" type="submit">Issue coins</button>{status && <span className="panel-count">{status}</span>}</form></section>;
}

function updateWithdrawal(id: string, status: string, accessToken: string) {
